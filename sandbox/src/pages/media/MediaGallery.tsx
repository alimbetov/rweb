...
interface MediaGalleryProps {
  offerId: number;
  linkedType?: string; // можно задать по умолчанию
}


const MediaGallery: React.FC<MediaGalleryProps> = ({ offerId, linkedType = "PRODUCT_GALLERY" }) => {
    const [page, setPage] = useState(0);  
  //const { linkedId, linkedType } = useParams<{ linkedId: string; linkedType: string }>();
  const linkedId = offerId;
  ...
  const toggleIsPublic = async (file: StoredFileDTO) => {
    try {
      ...
      await setFilePublic(file.id, newPublicState); // true => false и наоборот
      setRefresh((prev) => !prev); // обновим галерею
    } catch (error) {
      console.error("Ошибка при смене публичности файла:", error);
      alert("Не удалось изменить статус публичности файла.");
    }
  };
  ...
  const setAsMainImage = async (file: StoredFileDTO) => {
    try {
      ...
      alert("Файл установлен как главное изображение.");
    } catch (error) {
      console.error("Ошибка при установке главного изображения:", error);
      alert("Не удалось установить файл как главное изображение. Убедитесь, что файл публичный.");
    }
  };
