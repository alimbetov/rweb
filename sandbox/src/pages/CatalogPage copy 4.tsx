...
const CatalogPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row">
      {/* Sidebar */}
      <div
        className={`fixed md:static z-40 top-0 left-0 w-72 h-full bg-white shadow-lg p-4 transition-transform duration-300 transform md:translate-x-0 ${
              ...
              <div>
                {tab === "mine" && (
                  <OfferList
                    ...
                    other={false}
                    showStatusFilter={true} // ✅ только для "Мои офферы"
                  />
                )}
                {tab === "others" && (
                  <OfferList
                    productId={Number(selectedLeaf.id)}
                    other={true}
                    showStatusFilter={false} // ❌ нет фильтра по статусу
                  />
                )}
              </div>
            </div>
